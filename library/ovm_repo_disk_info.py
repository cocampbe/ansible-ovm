...
DOCUMENTATION = '''
---
module: ovm_repo_disk_info
short_description: This module will return the device file assocaited with a repository
description:
  - Module to get repository disk device file
...
def main():
    ...
    fileSystemObj = client.get(
        'FileSystem',
        repositoryObj['fileSystemId']['value'])
    
    if repositoryId is not None:
      if fileSystemObj is not None:
        result = dict(
            stdout=fileSystemObj['path'],
            rc=0,
            changed=True
        )
